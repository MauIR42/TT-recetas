...
char * struct_to_http_method(char * method, char * url, struct datos data, int weight );
...
int main(int argc, char **argv)
{
	...
	if(sockfd == -3){
		printf("El servidor tardo en responderno se pudo conectar con el servidor\n");
		return -1;
	}
...
char * struct_to_http_method(char * method, char * url, struct datos data, int weight){
	...
	char post_json_bone[] = "{\"time\" : \"%x:%x:%x_%x-%x-%x\", \"weight\": %d}\0";
	char post_json[50];

	char get_params_bone[] = "?time=%x:%x:%x_%x-%x-%x&weight=%d";
	char get_params[20];


	if(!strcmp("POST", method)){
		printf("POST petition\n");
		sprintf(post_json, post_json_bone, data.horas, data.minutos, data.segundos,data.dia,data.mes,data.anio,weight);
		// printf("json: %s\n", post_json);
		...
		printf("GET petition\n");
		sprintf(get_params,get_params_bone,data.horas, data.minutos, data.segundos,data.dia,data.mes,data.anio,weight);
